...
(function (global, factory) {
  ...
  Dep.prototype.depend = function depend() {
    if (Dep.target) {
      Dep.target.addDep(this); //将观察者放入集合中
      console.log(this);
    }
  };
  ...
  Dep.prototype.notify = function notify() {
    ...
    console.log("notify to update......"); //观察目标发生了变化
    ...
    for (var i = 0, l = subs.length; i < l; i++) {
      console.log(subs[i].update);
      subs[i].update(); //触发所有观察者
    }
  };
    ...
    def(arrayMethods, method, function mutator() {
      ...
      switch (method) {
        ...
        case "splice":
          ...
          console.log("weidehai slice=" + inserted);
          break;
      }
      if (inserted) {
  ...
  function observe(value, asRootData) {
    ...
    if (hasOwn(value, "__ob__") && value.__ob__ instanceof Observer) {
      ...
      console.log("create an observer instance for a value");
...

  {
    ...
    initProxy = function initProxy(vm) {
      console.log("initProxy::hasProxy" + hasProxy);
      if (hasProxy) {
  ...
  function resolveSlots(children, context) {
	...
	// weidehai 编译的时候，组件下面的所有内容会被放到slot中，slot是一个数组，里面就是所有子组件的vnode，如果我们需要渲染出这些子内容，就需要在render函数中手动添加这些vnode
	// render函数中的this.$slots.default===模板字符串中的<slot></slot>(其实模板字符串中的这个标签最后也会被编译成this.$slots.default)
    return slots;
  }
  ...
  function normalizeScopedSlot(normalSlots, key, fn) {
    // weidehai render函数中的this.$scopedSlots.default指向normalized
  ...
  /*  */
  // weidehai 将这个函数以_u的别名编译到render函数中
  function resolveScopedSlots(
    ...
    for (var i = 0; i < fns.length; i++) {
      ...
      if (Array.isArray(slot)) {
		...
		//将作用域插槽的key和其对应的渲染函数挂载到实例的ScopedSlots对象上
  ...
  function createFunctionalComponent(Ctor, propsData, data, contextVm, children) {
    alert("createFunctionalComponent");
    var options = Ctor.options;
    var props = {};
  ...
  // weidehai 之前只是把组件编译成了vnode，现在要通过vnode创建真实的组件
    ...
    Vue.prototype._render = function () {
      ...
      if (_parentVnode) {
        vm.$scopedSlots = normalizeScopedSlots(
          _parentVnode.data.scopedSlots,   // weidehai scopedSlots作为实例的data对象的数据
          vm.$slots,
          vm.$scopedSlots
        );
      }
      ...
      try {
        ...
        // weidehai 生成vnode,render函数调用createElemnt生成vnode
      } catch (e) {
  ...
  Watcher.prototype.get = function get() {
    pushTarget(this);
    console.log("push target to dep....");
    var value;
    var vm = this.vm;
    try {
  ...
  Watcher.prototype.addDep = function addDep(dep) {
    var id = dep.id;
    console.log("new depend id is " + id);
    console.log(this);
  ...
  function initMixin(Vue) {
    Vue.prototype._init = function (options) {
      console.log("invoke::new Vue()");
      console.log(options);
      console.log("vm 就是vue实例");
      var vm = this;
      console.log(vm);
      // a uid
      vm._uid = uid$3++;
  ...
  function initInternalComponent(vm, options) {
	...
	// weidehai嵌套组件编译的时候其实在父组件中已经把所有的层级关系都编译出来了，等到编译子组件的时候，子组件并不知道自己下面还有多少子节点，但是可以通过父组件来拿到
    var vnodeComponentOptions = parentVnode.componentOptions;
    opts.propsData = vnodeComponentOptions.propsData;
    opts._parentListeners = vnodeComponentOptions.listeners;
    opts._renderChildren = vnodeComponentOptions.children; // weidehai拿到自己下面需要渲染的子节点
  ...
  function initAssetRegisters(Vue) {
     ...
     */
    // weidehai创建资源注册方法，方法名在ASSET_TYPES数组中，方法挂载在Vue上（component方法就在其中）
    ASSET_TYPES.forEach(function (type) {
      Vue[type] = function (id, definition) {
        if (!definition) {
          ...
          if (type === "component" && isPlainObject(definition)) {
            log.warn("Vue.component");
            log.warn("Vue.component id=" + id);
            definition.name = definition.name || id;
            definition = this.options._base.extend(definition);
            // weidehai this.options._base指向Vue，调用了Vue的extend方法，返回VueComponent函数
          }
          if (type === "directive" && typeof definition === "function") {
            definition = { bind: definition, update: definition };
          }
          this.options[type + "s"][id] = definition;  // weidehai 将全局注册的组件放到Vue的options对象的comonents数组中
          // weidehai 全局注册的组件在Vue的options对象的comonents对象数组中，key是组件的名称，value是组件的参数，后面解析到模板使用了这个
          // 组件就会到这个里面通过name找对应的组件
          return definition;
        }
      };
    });
  ...
  function initGlobalAPI(Vue) {
    ...
    // weidehaiVue上的option最终会被合并到实例的$option上
    Vue.options._base = Vue;

    extend(Vue.options.components, builtInComponents);
    console.warn(Vue.options.components);
    initUse(Vue);
    initMixin$1(Vue);
    initExtend(Vue);
  ...
  console.log("vue defineProperty FunctionalRenderContext");
  ...
  function createPatchFunction(backend) {
    ...
    function createElm(vnode, insertedVnodeQueue, parentElm, refElm, nested, ownerArray, index) {
      ...
      if (isDef(tag)) {
        ...
        // weidehai 创建了真实的dom对象
        // nodeOps里面封装了原生的document方法
        vnode.elm = vnode.ns
    ...
    return function patch(oldVnode, vnode, hydrating, removeOnly) {
      ...
      const patchStartTime = new Date().getTime();
      ...
      log.warn("patch end");
      const patchEndTime = new Date().getTime();
      log.warn("patch waste time=" + (patchEndTime - patchStartTime));
      return vnode.elm;
    };
  }
  ...
  function createCompileToFunctionFn(compile) {
    ...
    return function compileToFunctions(template, options, vm) {
      ...
      console.log(typeof compiled.render);
      ...
      console.log(res.render);
      return (cache[key] = res);  //缓存，以模板字符串作为键值，下一次编译的时候，如果是同样的模板字符串就不需要在生成render函数了
    };
  }
  ...
  /*  */
  //createCompiler由createCompilerCreator生成，返回一个函数，执行这个函数，返回对象{compile: compile,compileToFunctions: createCompileToFunctionFn(compile)}
  var ref$1 = createCompiler(baseOptions);
  var compile = ref$1.compile;
  //createCompileToFunctionFn(compile) 传入一个函数并且最终返回一个函数
  ...
  Vue.prototype.$mount = function (el, hydrating) {
    ...
    // weidehai 如果自己写了render函数，就不会去拿template来编译
    // weidehai 如果没有写编译函数，就会去拿template
    // template来源有两个：1、直接写在创建Vue实例里面的option对象中{template:xxxxx}
    //                    2、用option定义的el，用原生document方法document.queryselector(el).getOuterHTML拿到html模板
    // 总结就是：render函数>option.template>option.el
    if (!options.render) {
      ...
      if (template) {
        ...
        //compileToFunctions函数将template编译成render函数
        console.log("weidehai" + options.delimiters);
